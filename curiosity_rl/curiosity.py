...
class NavRLModel(nn.Module):
    def __init__(self):
        super().__init__()
        # Encode lidar and dynamic obstacle features; robot_state will be concatenated raw.
...
class HICM():
    ...
    def __init__(self, config, robot_state_dim, dyn_obs_shape, static_obs_shape, device, lr, scaling_factor, policy="tree_search_rl", continuous_action=False, action_dim=None):
        # scaling_factor: intrinsic reward weight; continuous_action toggles inverse/forward heads.
        ...
        if policy == "ponav":
            ...
            # Dummy forward pass to initialize Lazy modules so shapes are fixed.
            dummy_robot = torch.zeros(1, robot_state_dim).to(device)
            dummy_lidar = torch.zeros(1, *static_obs_shape).to(device)
            dummy_dyn = torch.zeros(1, *dyn_obs_shape).to(device)
    ...
    def get_exploration_optimizer(self):
        # Optimizer only for curiosity modules; policy optimizers stay separate.
    ...
    def compute_poincare_distance(self, x, y):
        # Forward loss uses Poincaré距离；先投影再裁剪防 NaN/Inf。

...
def imitation_learning(model_to_init, policy, model_name, policy_config, SB3_vars, robot, env, load_imit_file):
	...
	if load_imit_file is False:
		explorer.run_k_episodes(
			...
			require_success_for_memory=True)
		if len(memory) == 0:
			logging.warning('No successful imitation-learning rollouts were collected; retrying with fallback storage of failed episodes.')
			explorer.run_k_episodes(
				il_episodes,
				'train',
				update_memory=True,
				imitation_learning=True,
				require_success_for_memory=False)
		if len(memory) == 0:
			raise RuntimeError('Imitation learning memory is still empty. Consider using --load_imit_file or adjusting the IL policy/config.')

...
def evaluate(
    ...
    # Defer heavy / optional imports to runtime to avoid import errors when not using Isaac backend
...
def vec_to_new_frame(vec, goal_direction):
    ...
    # goal direction y
    goal_direction_y = torch.linalg.cross(z_direction.expand_as(goal_direction_x), goal_direction_x)
    goal_direction_y /= goal_direction_y.norm(dim=-1, keepdim=True)
    
    # goal direction z
    goal_direction_z = torch.linalg.cross(goal_direction_x, goal_direction_y)

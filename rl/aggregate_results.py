...
def get_scenario_name(args, env_config_file, policy_config, env, robot):
    ...
    elif 'ponav' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_navrl', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'navrl_{:}_{:}'.format(env_id, args.num_humans)
    else:
        scenario_name = 'null'
...
def main():
    ...
    parser.add_argument('--repeat', type=int, default=1, help='Number of times to repeat each policy')
    parser.add_argument('--log_file', type=str, default='aggregate_results.log', help='Log file to store results')
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s, %(levelname)s: %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S',
                        handlers=[logging.FileHandler(args.log_file, mode='a'), logging.StreamHandler()])
    ...
    for repeat_idx in range(args.repeat):
        ...
        if args.num_humans != -1:
            env_config.set('sim', 'human_num', str(args.num_humans))
        else:
        ...
        if args.policy is not None:
            ...
            logging.info('Using policy: {:}'.format(args.policy))
        ...
        # Set the name
        if args.policy == 'campc':
            tc_name = '{:}_tc_{:}_rep_{:}'.format(scenario_name, args.test_case, repeat_idx)
        ...
        logging.info('[TEST PHASE] About to start test case %i, repeat %i.', viz_test_case, repeat_idx)
        done = False
        states = []
        actions_array = []
        min_dist = []
        ...
        summ_dict = { 'test_case' : viz_test_case,
                    ...
                    'coll_freq': coll_log / env.tot_env_steps if env.tot_env_steps > 0 else 0,
                    'wall_coll_freq': wall_coll_log / env.tot_env_steps if env.tot_env_steps > 0 else 0,
                    'frozen_freq': frozen_log / env.tot_env_steps if env.tot_env_steps > 0 else 0,
                    'too_close_freq' : danger_log / env.tot_env_steps if env.tot_env_steps > 0 else 0,
                    'campc/campc_data': campc_data,
                    }
        # 记录基本数据到日志
        logging.info('==== Experiment Result (repeat %d) ====', repeat_idx)
        ...
        # logging.info('video_file: {:}'.format(summ_dict['video_file'])) # 已去除视频功能

...
def get_scenario_name(args, env_config_file, policy_config, env, robot):
    ...
    elif 'sarl' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_sarl', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'sarl_{:}_{:}'.format(env_id, args.num_humans)
    elif 'sicnav_acados' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_sicnav_acados', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'sicnav_acados_{:}_{:}'.format(env_id, args.num_humans)
    elif 'cnn_policy' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_sicnav_cnnrl', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'sicnav_cnnrl_{:}_{:}'.format(env_id, args.num_humans)
    elif 'dsrnn_obs_vertex' in args.policy:
        save_dir = os.path.join(os.getcwd(), 'results_sicnav_dsrnn', env_id)
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        scenario_name = 'sicnav_dsrnn_{:}_{:}'.format(env_id, args.num_humans)
    else:
        scenario_name = 'null'
...
def main():
    ...
    # Disable video recording/rendering
    parser.add_argument('--no_video', default=False, action='store_true', help='Disable video rendering and saving')
    ...
    if args.policy is not None:
        ...
        print('Using policy: {:}'.format(args.policy))
        # return 

...
class PONav(Policy):
    ...
    def __init__(self):
        ...
        # 根本性安全过滤参数（CBF风格）
        self.d_safe_base = 0.05   # 额外安全距离，与机器人半径相加
        self.alpha_cbf = 1.0     # 约束收缩系数（越大越保守）
        self.qp_max_iters = 50    # 半空间投影迭代次数
        self.progress_floor = 0.0  # 禁止后退/保持最小前向投影
        self.safety_space = 0.05
        self.time_horizon_obst = 10.0
    ...
    def predict(self, env_state):
        ...
        # 仍使用 LiDAR 半空间过滤作为后备
        try:
            if not applied_orca_guided and range_matrix is not None and range_matrix.size > 0:
    ...
    def _apply_safety_filter_orca_guided(self, v_des, pos, static_lines, robot_radius, safety_space, tau_obs, vmax, target_dir):
        ...
        for seg in static_lines or []:
            ...
            if dist < min_dist_debug: min_dist_debug = dist
            ...
            if dist < 1e-8:
                ...
                n = diff / dist
            c = k * (dist - d_safe)
            if c < 0: c = 0.0 # No pushback, just stop approach
            ...
            for endpoint in (a, b):
                ...
                if dist_e < 1e-8:
                    ...
                    n_e = diff_e / dist_e
                c_e = k * (dist_e - d_safe)
                if c_e < 0: c_e = 0.0 # No pushback, just stop approach
        ...
        # if min_dist_debug < d_safe + 0.5:
        #    print(f"DEBUG: min_dist={min_dist_debug:.3f}, d_safe={d_safe:.3f}, v_in={v_des}, v_out={v}")

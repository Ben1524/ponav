...
def test(save_stats_path, model_name, model_path, env_config_file, policy_config_file, MPC_test):
    ...
    read_files = env_config.read(env_config_file)
    print(f"Env config file: {env_config_file}, Read: {read_files}")
    
    policy_config = configparser.RawConfigParser()
    read_files = policy_config.read(policy_config_file)
    print(f"Policy config file: {policy_config_file}, Read: {read_files}")
    print(policy_config)
    ...
    if MPC_test is False:
        ...
        # Patch set_parameters to allow loading model without optimizer
        from stable_baselines3.common.base_class import BaseAlgorithm
        original_set_parameters = BaseAlgorithm.set_parameters
        def custom_set_parameters(self, load_path_or_dict, exact_match=True, device="auto"):
            return original_set_parameters(self, load_path_or_dict, exact_match=False, device=device)
        BaseAlgorithm.set_parameters = custom_set_parameters
        
        model = DQN.load(model_path, env=env)
        
        BaseAlgorithm.set_parameters = original_set_parameters

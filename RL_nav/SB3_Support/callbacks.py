...
def _extract_info_value(entry):
    """Return a scalar/boolean value from an info entry.

    Environment infos may be custom objects exposing .val attributes or plain
    Python/numpy scalars. This helper normalizes the value for logging logic.
    """
    if hasattr(entry, 'val'):
        return entry.val
    if isinstance(entry, np.ndarray):
        return float(entry.squeeze()) if entry.size == 1 else entry
...
class CustomCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        done_entry = info_dict.get("Done", False)
        eps_complete = bool(_extract_info_value(done_entry))
        for key, value in info_dict.items():
            if key=="Done" or key == "episode" or key == "terminal_observation":
                continue
            # record raw values
            scalar_val = _extract_info_value(value)
            self.logger.record(key, scalar_val)
            ...
            if eps_complete:
                ...
                if key in self.over_time_rewards:
                    update_val = 1 if scalar_val !=0 else 0

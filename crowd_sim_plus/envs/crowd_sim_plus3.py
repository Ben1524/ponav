...
class CrowdSimPlus(gym.Env):
    ...
    def generate_static_obstacles(self, rule, static_obstacles=None):
        ...
        # 增加一道墙
        self.static_obstacles.append([(-self.rect_width * 0.3, 0), (self.rect_width, 0)])
        # self.static_obstacles.append([(-self.rect_width * 0.3, -self.rect_height * 0.4), (self.rect_width * 0.5, self.rect_height * 0.4)])
        # Add irregular obstacles
        num_irregular = 0
        ...
        for _ in range(num_irregular):
            ...
            while attempts < 100:
                ...
                if not overlap:
                    ...
                    poly_segments = self.generate_irregular_polygon(vertices, radius, (cx, cy))
    ...
    def generate_random_human_position(self, human_num, rule, rng):
        """
        Generate human position randomly, avoiding static obstacles.
        """
        self.humans = []
        for i in range(human_num):
            ...
            if self.randomize_attributes:
                ...
                human.radius = rng.uniform(0.3, 0.5)
            
            while True:
                # Generate random position within the rectangle
                px = (rng.random() - 0.5) * self.rect_width
                py = (rng.random() - 0.5) * self.rect_height
                
                collide = False
                # Check collision with robot and other humans
                for agent in [self.robot] + self.humans:
                    if norm((px - agent.px, py - agent.py)) < human.radius + agent.radius + 0.2: # Add some buffer
                        collide = True
                        break
                ...
                if not collide:
                    ...
                    for line in self.static_obstacles:
                        dist = point_to_segment_dist(line[0][0], line[0][1], line[1][0], line[1][1], px, py)
                        if dist < human.radius + 0.2: # Add some buffer
                            collide = True
                            break
            ...
            while True:
                # Generate random goal within the rectangle
                gx = (rng.random() - 0.5) * self.rect_width
                gy = (rng.random() - 0.5) * self.rect_height
                
                collide = False
                # Check collision with static obstacles for goal (optional but good practice)
                for line in self.static_obstacles:
                    dist = point_to_segment_dist(line[0][0], line[0][1], line[1][0], line[1][1], gx, gy)
                    if dist < human.radius + 0.2:
                        collide = True
                        break
    ...
    def render(self, mode='human', output_file=None, joint_state=None, debug=False):
        ...
        if mode == 'joint_state':
            ...
            fig, ax = plt.subplots(figsize=(7, 7))
            if hasattr(self, 'rect_width') and self.rect_width is not None:
                limit = max(self.rect_width, self.rect_height) / 2.0 + 1

...
class CrowdSimPlus(gym.Env):
    ...
    def configure(self, config, occlusion_override=False, occlusion_val=False, policy_config=None):
        ...
        # From crowd sim:
        self.config = config
        self.drop_prob_dyn = config.getfloat('env', 'drop_prob_dyn', fallback=0.0)
        ...
        # Backwards-compatibility: some code references self.discomfort_dist directly
        # (older code paths). Expose it as an attribute for convenience.
        self.discomfort_dist = float(self.rewards.get('discomfort_dist', 0.2))
        ...
        # 是否随机化机器人目标（在 reset 中基于 case seed 随机化），默认关闭
        self.randomize_robot_goal = config.getboolean('env', 'randomize_robot_goal', fallback=True)
        if self.randomize_robot_goal:
            logging.info('[CrowdSimPlus] Robot goal will be randomized per case seed')
        else:
            logging.info('[CrowdSimPlus] Robot goal will NOT be randomized')
    ...
    def set_robot(self, robot):
        self.robots = [deepcopy(robot) for _ in range(self.robot_num)]
        self.robot = robot
        self.robot_collisions = [0 for _ in range(self.robot_num)]

    def set_robot_num(self, robot_num):
        """_summary_

        :param robot_num: _description_
        """
    ...
    def generate_irregular_polygon(self, num_vertices, radius, center):
    ...
    def generate_static_obstacles(self, rule, static_obstacles=None):
        ...
        if static_obstacles is None:
            ...
            if rule == 'hallway_static' or rule == 'hallway_static_with_back' or rule == "hallway_bottleneck" or rule == "hallway_squeeze":
                ...
                self.door_width = 0.5 * self.rect_width if rule == "hallway_squeeze" else 2.0
                ...
                if rule == "hallway_squeeze":
                    ...
                    if 'hallway_static' in rule:
                        ...
                        y_mid = 0
                        hallway_door = [
                            # [(-self.rect_width * 0.5, y_mid),(self.rect_width*0.2 , y_mid)], #2
            ...
            num_irregular = 10  # 这是我新加的参数，用于控制随机多边形障碍物的数量
            existing_obstacles = [] # List of (x, y, radius)

            for _ in range(num_irregular):
                attempts = 0
                while attempts < 100:
                    cx = random.uniform(-self.rect_width * 0.4, self.rect_width * 0.4)
                    cy = random.uniform(-self.rect_height * 0.4, self.rect_height * 0.4)
                    radius = random.uniform(0.5, 1.5)
                    
                    # Check overlap with existing obstacles
                    overlap = False
                    for (ex_x, ex_y, ex_r) in existing_obstacles:
                        dist = np.linalg.norm([cx - ex_x, cy - ex_y])
                        if dist < (radius + ex_r + 0.8): # 0.8 buffer for "not too close"
                            overlap = True
                            break
                    ...
                    if not overlap:
                        ...
                        if shape_type == 'circle':
                            ...
                            side_ratio = random.uniform(0.5, 1.0)
                            w = 2 * radius / np.sqrt(1 + side_ratio**2)
                            h = w * side_ratio
                            ...
                            for v in vs:
                                ...
                                points.append((cx + rotated_v[0], cy + rotated_v[1]))
                        
                        points.append(points[0])
                        for i in range(len(points) - 1):
                            poly_segments.append([points[i], points[i+1]])

                        self.static_obstacles.extend(poly_segments)
                        existing_obstacles.append((cx, cy, radius))
                        break
    ...
    def _poisson_disk_samples(self, xmin, xmax, ymin, ymax, min_dist, rng, k=30, target_num=None):
        """Simple Bridson Poisson-disk sampler on an axis-aligned rectangle.

        Returns up to target_num points (or all found points if target_num is None).
        Uses rng (np.random.Generator) for randomness.
        """
        import math
        width = xmax - xmin
        height = ymax - ymin
        ...
        gx = max(1, int(math.ceil(width / cell_size)))
        gy = max(1, int(math.ceil(height / cell_size)))
        grid = [[-1 for _ in range(gy)] for _ in range(gx)]
        ...
        # initial point
        p0 = (rng.uniform(xmin, xmax), rng.uniform(ymin, ymax))
        samples.append(p0)
        process.append(p0)
        ix, iy = grid_coords(p0)
        grid[ix][iy] = 0

        while process and (target_num is None or len(samples) < target_num):
            idx = rng.integers(0, len(process))
            p = process[idx]
            found = False
            for _ in range(k):
                radius = rng.uniform(min_dist, 2 * min_dist)
                theta = rng.uniform(0, 2 * math.pi)
                nx = p[0] + radius * math.cos(theta)
                ny = p[1] + radius * math.sin(theta)
                if not (xmin <= nx <= xmax and ymin <= ny <= ymax):
                    continue
                nix, niy = grid_coords((nx, ny))
                ok = True
                # check neighbors
                for iix in range(max(0, nix - 2), min(gx, nix + 3)):
                    for iiy in range(max(0, niy - 2), min(gy, niy + 3)):
                        gidx = grid[iix][iiy]
                        if gidx != -1:
                            sx, sy = samples[gidx]
                ...
                if ok:
                    ...
                    process.append((nx, ny))
                    gix, giy = nix, niy
                    grid[gix][giy] = len(samples) - 1
                    found = True
                    break
            if not found:
                # remove point
    ...
    def generate_square_crossing_human(self, rng):
        ...
        if rng.random() > 0.5:
            ...
            sign = 1
        while True:
            # 产生0-1之间的随机数，再乘以 square_width 的一半，得到人的起始位置 x 坐标
            rd = rng.uniform(0, 1)
            px = rng.random() * self.square_width * rd * sign
            py = (rng.random() - rd) * self.square_width
            collide = False
            for agent in [self.robot] + self.humans:
    ...
    def generate_hallway_human(self, rng):
        ...
        while True:
            ...
            px = (rng.random()) * 0.5 * wor_sign * (self.rect_width / 2- human.radius * 2)
            py = (rng.random()) * 0.25 * dir_sign * (self.circle_radius / 2)  * (effective_rect_height - human.radius * 2)
            collide = False
            ...
            if collide:
                ...
                continue
            # gx = px + rng.random() * 0.5 *self.rect_width
            # gy = py + dir_sign * self.circle_radius * (effective_rect_height - human.radius * 2) * 0.5
            gx = (rng.random()) * 0.5 * cross_sign * (self.rect_width - human.radius * 2)
            gy = (rng.random()) * 0.1 * -dir_sign * self.circle_radius * (effective_rect_height - human.radius * 2)# + -dir_sign * (self.rect_height - human.radius * 2)
            collide = False
            for agent in [self.robot] + self.humans:
            # for agent in [self.robot]:
    ...
    def reset(self, phase='test', test_case=None, return_stat=False, testing_human_num=-1, SB3_override=False):
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            self.robot.set(-self.circle_radius/2, -self.circle_radius, self.circle_radius/2, self.circle_radius, 0, 0, np.pi / 2)
            if self.case_counter[phase] >= 0:
                ...
                # 根据配置开关决定是否随机化机器人起点与目标（基于场景种子可重复）
                if getattr(self, 'randomize_robot_goal', False):
                    try:
                        margin = self.robot.radius + 0.1
                        x_min = -self.rect_width * 0.5 + margin
                        x_max = self.rect_width * 0.5 - margin
                        # 固定在场景上下边（与原始实现一致）：起点 y 为 -circle_radius，目标 y 为 +circle_radius
                        ...
                        for _ in range(max_attempts):
                            px_c = rng.uniform(x_min, x_max)
                            gx_c = rng.uniform(x_min, x_max)
                            # 与所有人及其目标检测冲突
                            collide = False
                            for h in self.humans:
                                # 检查与人起点/目标的最小距离
                                if np.linalg.norm((px_c - h.px, py - h.py)) < (self.robot.radius + h.radius + self.rewards.get('discomfort_dist', 0.2)):
                                    collide = True
                                    break
                                if np.linalg.norm((gx_c - h.gx, gy - h.gy)) < (self.robot.radius + h.radius + self.rewards.get('discomfort_dist', 0.2)):
                                    collide = True
                                    break
                            # 保证起点与目标之间有合理间隔
                            if not collide and np.linalg.norm((px_c - gx_c, py - gy)) > (self.robot.radius + 0.5):
                                px = px_c
                                gx = gx_c
                                break
                        else:
                            # 若未找到合适位置，回退到原始固定设置
                            px, py, gx, gy = 0, -self.circle_radius, 0, self.circle_radius

                        # 应用新的机器人位姿（覆盖之前的默认设置）
                        self.robot.set(px, py, gx, gy, 0, 0, np.pi / 2)
                    except Exception as e:
                        # 若发生意外，保留原始硬编码设置（已在前面设置过）
                        logging.warn(f"Failed to randomize robot goal, using default. Error: {e}")
        ...
        if self.robot.sensor == 'coordinates':
            ...
            if SB3_override is False and self.SB3:
                ...
                human_state_list = self.get_human_observable_states(original=False)
                ...
                self.robot_prev_dist_to_goal = np.linalg.norm(robot_prev_pos - self.robot_goal_pos)
                ob = self.get_human_observable_states(original=True)
    ...
    def step(self, action, update=True, dummy_start=False):
       ...
       # get constrained robot action + check for static collision
        ...
        if update:
            ...
            # If a human has reached its final goal, assign a new goal so they continue moving
            for i, human in enumerate(self.humans):
                try:
                    if human.reached_destination():
                        # sample a new final goal for this human within map bounds
                        rng = np.random.default_rng()
                        margin = human.radius + 0.2
                        x_min = -self.rect_width * 0.5 + margin
                        x_max = self.rect_width * 0.5 - margin
                        y_min = -self.rect_height * 0.5 + margin
                        y_max = self.rect_height * 0.5 - margin
                        # ensure new goal is meaningfully far from current position
                        max_attempts = 200
                        found = False
                        for _attempt in range(max_attempts):
                            gx_c = float(rng.uniform(x_min, x_max))
                            gy_c = float(rng.uniform(y_min, y_max))
                            # avoid goals that are too close to current position or other agents' goals
                            if np.linalg.norm((gx_c - human.px, gy_c - human.py)) < max(0.5, human.v_pref * 0.5):
                                continue
                            too_close = False
                            for other in [self.robot] + [h for j, h in enumerate(self.humans) if j != i]:
                                if np.linalg.norm((gx_c - other.px, gy_c - other.py)) < (human.radius + other.radius + self.rewards.get('discomfort_dist', 0.2)):
                                    too_close = True
                                    break
                            if too_close:
                                continue
                        ...
                        if found:
                            human.final_gx = gx_c
                            human.final_gy = gy_c
                            # update intermediate/intermediate goal used by hallway logic
                            try:
                                human.set_g_xy(human.px, human.py)
                            except Exception:
                                # best-effort: set immediate goal
                                human.gx = human.final_gx
                                human.gy = human.final_gy
                        else:
                            # fallback: mirror current goal across origin to keep moving
                            human.final_gx = -human.final_gx
                            human.final_gy = -human.final_gy
                            try:
                                human.set_g_xy(human.px, human.py)
                            except Exception:
                                human.gx = human.final_gx
                    ...
                    # Guard: if any error occurs here, don't let it crash the environment step
                    logging.debug("Failed to reassign human goal", exc_info=True)
    ...
    def get_human_observable_states(self, original=True):
        import numpy as np
        from crowd_sim_plus.envs.utils.state_plus import ObservableState
        ob = []
        for human in self.humans:
            if np.random.rand() < getattr(self, 'drop_prob_dyn', 0.0):
                ob.append(ObservableState(1000, 1000, 0, 0, 0.3))
            else:
                ob.append(human.get_observable_state(original=original))
        return ob
    ...
    def render(self, mode='human', output_file=None, joint_state=None, debug=False):
        ...
        if mode == 'joint_state':
            ...
            if self.static_obstacles:
                ...
                ax.set_ylim(-self.rect_height * 0.5 , self.rect_height * 0.5)
                ax.set_xlim( -self.rect_width * 0.5 , self.rect_width * 0.5)
